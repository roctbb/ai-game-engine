...
def _required_slot_key(client, game_id: str) -> str:
...
def _create_ready_team(client, game_id: str, user_id: str) -> str:
    team = client.post(
        '/api/v1/teams',
        json={
            'game_id': game_id,
    ...
    slot = _required_slot_key(client, game_id)
    update = client.put(
...
def test_single_task_run_endpoint_creates_and_queues_run(client) -> None:
    ...
    team_id = _create_ready_team(client, game_id=game_id, user_id='runner')
...
def test_single_task_run_with_empty_required_slot_does_not_block_next_attempt(client) -> None:
    ...
    maze_game_id = _find_game_id(client, 'maze_escape_v1')
    coins_game_id = _find_game_id(client, 'coins_right_down_v1')
    ...
    ready_team_id = _create_ready_team(client, game_id=coins_game_id, user_id=user_id)
...
def test_single_task_attempts_endpoint_supports_filter_and_limit(client) -> None:
    game_id = _find_game_id(client, 'maze_escape_v1')
    team_a = _create_ready_team(client, game_id=game_id, user_id='attempt-a')
    team_b = _create_ready_team(client, game_id=game_id, user_id='attempt-b')
    ...
    filtered = client.get(f'/api/v1/single-tasks/{game_id}/attempts?requested_by=attempt-a&limit=1')
...
def test_single_task_attempts_endpoint_supports_status_and_offset(client) -> None:
    game_id = _find_game_id(client, 'maze_escape_v1')
    team = _create_ready_team(client, game_id=game_id, user_id='offset-user')
    ...
    finished_only = client.get(f'/api/v1/single-tasks/{game_id}/attempts?status=finished')
    ...
    paged = client.get(f'/api/v1/single-tasks/{game_id}/attempts?limit=1&offset=1')
...
def test_single_task_stop_and_logs_endpoints(client) -> None:
    game_id = _find_game_id(client, 'maze_escape_v1')
    team_id = _create_ready_team(client, game_id=game_id, user_id='stopper')
    ...
    logs = client.get(f"/api/v1/single-task-attempts/{run_for_logs['run_id']}/logs")
    assert logs.status_code == 200
    payload = logs.json()
    assert payload['attempt_id'] == run_for_logs['run_id']
    assert payload['lines'] == ['compile_error: invalid syntax']

    lookup = client.get(f"/api/v1/single-task-attempts/{run_for_logs['run_id']}")
