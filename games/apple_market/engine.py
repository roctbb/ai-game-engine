...
_CAPACITY = 2
_SLOTS = ("north", "south")
_BASES = {"north": (1, 1), "south": (10, 10)}
_STARTS = {"north": (1, 2), "south": (10, 9)}
_RANDOM_WALLS = 18
_APPLES_TOTAL = 14
...
def run(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    assert isinstance(walls, set) and isinstance(apples, set)

    positions = dict(_STARTS)
    carrying = {slot: 0 for slot in _SLOTS}
    delivered = {slot: 0 for slot in _SLOTS}
    invalid = {slot: 0 for slot in _SLOTS}
    turns = 0
    frames = [_frame(0, "running", positions, walls, apples, carrying, delivered, invalid)]

    for turn in range(_MAX_TURNS):
        if not apples and not any(carrying.values()):
            break
        ...
        for slot in _SLOTS:
            ...
            x, y = positions[slot]
            action = _safe_call(fn, x, y, _board(walls, apples, positions, slot), carrying[slot], slot)
        ...
        if intents["north"] == intents["south"]:
            intents = {slot: positions[slot] for slot in _SLOTS}
            events.append({"type": "collision_bounce", "tick": turn + 1})
        elif intents["north"] == positions["south"] and intents["south"] == positions["north"]:
            intents = {slot: positions[slot] for slot in _SLOTS}
        ...
        turns = turn + 1
        frames.append(_frame(turns, "running", positions, walls, apples, carrying, delivered, invalid))

    compile_errors = {slot: err for slot, (_fn, err) in bots.items() if err}
    slot_scores = {slot: delivered[slot] * 120 + carrying[slot] * 30 - invalid[slot] * 10 for slot in _SLOTS}
    ...
    frames.append(_frame(len(frames), "finished", positions, walls, apples, carrying, delivered, invalid, slot_scores))
...
def _build_map(context: dict[str, Any]) -> dict[str, object]:
    ...
    for _attempt in range(500):
        ...
        shared = sorted((_reachable_cells(_BASES["north"], walls) & _reachable_cells(_BASES["south"], walls)) - blocked)
        if len(shared) < _APPLES_TOTAL + 8:
            continue
        rng.shuffle(shared)
        return {"walls": walls, "apples": set(shared[:_APPLES_TOTAL])}
...
def _board(walls: set[tuple[int, int]], apples: set[tuple[int, int]], positions: dict[str, tuple[int, int]], viewer: str) -> list[list[int]]:
    ...
    bx, by = _BASES[viewer]
    board[bx][by] = 2
    opponent = "south" if viewer == "north" else "north"
...
def _team_ids(ctx: dict[str, Any]) -> dict[str, str]:
    ...
    return {"north": str(team_id) if isinstance(team_id, str) and team_id else "team-north", "south": "team-south"}
...
def _frame(tick: int, phase: str, positions: dict[str, tuple[int, int]], walls: set[tuple[int, int]], apples: set[tuple[int, int]], carrying: dict[str, int], delivered: dict[str, int], invalid: dict[str, int], slot_scores: dict[str, int] | None = None) -> dict[str, object]:
    frame: dict[str, object] = {
        "board": _board(walls, apples, positions, "north"),
        "boards": {slot: _board(walls, apples, positions, slot) for slot in _SLOTS},
        "width": _WIDTH,
        "height": _HEIGHT,
