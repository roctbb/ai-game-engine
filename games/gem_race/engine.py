...
def run(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    turns = 0
    frames = [_frame(0, "running", positions, walls, gems, collected, invalid, labels=role_name)]
    ...
    for turn in range(_MAX_TURNS):
        ...
        for slot in _SLOTS:
            ...
            x, y = positions[slot]
            board = _board(walls=walls, gems=gems, positions=positions, viewer=slot)
        ...
        for slot in _SLOTS:
            if positions[slot] in gems:
                ...
                collected[slot] += 1
                events.append({"type": "gem", "tick": turn + 1, "slot": slot, "x": positions[slot][0], "y": positions[slot][1]})

        turns = turn + 1
        frames.append(_frame(turns, "running", positions, walls, gems, collected, invalid, labels=role_name))
    ...
    frames.append(_frame(len(frames), "finished", positions, walls, gems, collected, invalid, slot_scores, labels=role_name))
...
def _build_map(context: dict[str, Any]) -> dict[str, object]:
    seed = _map_seed(context, "gem_race_offline")
    ...
    for _attempt in range(400):
        ...
        shared = sorted((red_reachable & blue_reachable) - set(_STARTS.values()))
        if len(shared) < _GEMS_TOTAL + 8:
            continue
        rng.shuffle(shared)
        return {"walls": walls, "gems": set(shared[:_GEMS_TOTAL])}
    walls = set(_BORDER_WALLS)
    shared = sorted(_reachable_cells(_STARTS["red"], walls) & _reachable_cells(_STARTS["blue"], walls) - set(_STARTS.values()))
    rng.shuffle(shared)
    return {"walls": walls, "gems": set(shared[:_GEMS_TOTAL])}
...
def _placements(role_team: dict[str, str], slot_scores: dict[str, int]) -> dict[str, int]:
    ...
    return {role_team[slot]: index + 1 for index, slot in enumerate(ordered)}
...
def _frame(
    ...
    board = _board(walls=walls, gems=gems, positions=positions, viewer="red")
    frame: dict[str, object] = {
        "board": board,
        "width": _WIDTH,
        "height": _HEIGHT,
        "labels": {slot: (labels or {}).get(slot, slot) for slot in _SLOTS},
        "positions": {slot: {"x": x, "y": y} for slot, (x, y) in positions.items()},
        "gems_left": len(gems),
        "collected": collected,
        "invalid_moves": invalid,
    }
