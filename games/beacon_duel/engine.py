...
def run(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    beacons = game_map["beacons"]
    assert isinstance(walls, set) and isinstance(beacons, set)
    positions = dict(_STARTS)
    activated = {slot: 0 for slot in _SLOTS}
    invalid = {slot: 0 for slot in _SLOTS}
    turns = 0
    frames = [_frame(0, "running", positions, walls, beacons, activated, invalid)]
    ...
    for turn in range(_MAX_TURNS):
        ...
        for slot in _SLOTS:
            ...
            x, y = positions[slot]
            action = _safe_call(fn, x, y, _board(walls, beacons, positions, slot), slot)
        ...
        positions = intents
        for slot in _SLOTS:
            if positions[slot] in beacons:
                beacons.remove(positions[slot])
                activated[slot] += 1
                events.append({"type": "beacon", "tick": turn + 1, "slot": slot})
        turns = turn + 1
        frames.append(_frame(turns, "running", positions, walls, beacons, activated, invalid))

    compile_errors = {slot: err for slot, (_fn, err) in bots.items() if err}
    slot_scores = {slot: activated[slot] * 100 - invalid[slot] * 10 for slot in _SLOTS}
    ...
    metrics: dict[str, object] = {
        "turns": turns,
        "winner_slot": max(_SLOTS, key=lambda slot: (slot_scores[slot], activated[slot])),
        "winner_slots": _winner_slots(slot_scores),
        "is_tie": _is_tie(slot_scores),
        "activated": activated,
        "beacons_left": len(beacons),
        "beacons_total": _BEACONS_TOTAL,
    ...
    frames.append(_frame(len(frames), "finished", positions, walls, beacons, activated, invalid, slot_scores))
...
def _build_map(context: dict[str, Any]) -> dict[str, object]:
    ...
    for _attempt in range(500):
        ...
        if len(shared) < _BEACONS_TOTAL + 8:
            continue
        rng.shuffle(shared)
        return {"walls": walls, "beacons": set(shared[:_BEACONS_TOTAL])}
...
def _board(walls: set[tuple[int, int]], beacons: set[tuple[int, int]], positions: dict[str, tuple[int, int]], viewer: str) -> list[list[int]]:
...
def _team_ids(ctx: dict[str, Any]) -> dict[str, str]:
    ...
    return {"white": str(team_id) if isinstance(team_id, str) and team_id else "team-white", "black": "team-black"}
...
def _frame(tick: int, phase: str, positions: dict[str, tuple[int, int]], walls: set[tuple[int, int]], beacons: set[tuple[int, int]], activated: dict[str, int], invalid: dict[str, int], slot_scores: dict[str, int] | None = None) -> dict[str, object]:
    frame: dict[str, object] = {
        "board": _board(walls, beacons, positions, "white"),
        "boards": {slot: _board(walls, beacons, positions, slot) for slot in _SLOTS},
        "width": _WIDTH,
        "height": _HEIGHT,
