...
_MAX_TURNS = 110
_SLOTS = ("orange", "cyan")
_STARTS = {"orange": (1, 1), "cyan": (11, 11)}
_CHECKPOINTS_TOTAL = 5
_RANDOM_WALLS = 22
_DELTAS = {
    "up": (0, -1),
    "down": (0, 1),
...
def run(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    checkpoints = game_map["checkpoints"]
    assert isinstance(walls, set) and isinstance(checkpoints, list)

    positions = dict(_STARTS)
    progress = {slot: 0 for slot in _SLOTS}
    invalid = {slot: 0 for slot in _SLOTS}
    turns = 0
    frames = [_frame(0, "running", walls, checkpoints, positions, progress, invalid)]

    for turn in range(_MAX_TURNS):
        if all(progress[slot] >= _CHECKPOINTS_TOTAL for slot in _SLOTS):
            break
        ...
        for slot in _SLOTS:
            ...
            x, y = positions[slot]
            target = _target_for(slot, checkpoints, progress[slot])
            board = _board(walls, positions, slot, target)
            action = _safe_call(fn, x, y, board, progress[slot])
        ...
        if intents["orange"] == intents["cyan"]:
            intents = {slot: positions[slot] for slot in _SLOTS}
            events.append({"type": "collision_bounce", "tick": turn + 1})
        elif intents["orange"] == positions["cyan"] and intents["cyan"] == positions["orange"]:
            intents = {slot: positions[slot] for slot in _SLOTS}
        ...
        turns = turn + 1
        for slot in _SLOTS:
            if progress[slot] < _CHECKPOINTS_TOTAL and positions[slot] == _target_for(slot, checkpoints, progress[slot]):
                progress[slot] += 1
                events.append({"type": "checkpoint", "tick": turns, "slot": slot, "count": progress[slot]})
        frames.append(_frame(turns, "running", walls, checkpoints, positions, progress, invalid))

    compile_errors = {slot: err for slot, (_fn, err) in bots.items() if err}
    slot_scores = {slot: progress[slot] * 120 - invalid[slot] * 8 - turns for slot in _SLOTS}
    ...
    winner_slot = max(_SLOTS, key=lambda slot: (progress[slot], slot_scores[slot]))
    metrics: dict[str, object] = {
        ...
        "progress": progress,
        "checkpoints_total": _CHECKPOINTS_TOTAL,
    ...
    frames.append(_frame(len(frames), "finished", walls, checkpoints, positions, progress, invalid, slot_scores))
...
def _build_map(context: dict[str, Any]) -> dict[str, object]:
    ...
    for _attempt in range(500):
        ...
        shared = _reachable_cells(_STARTS["orange"], walls) & _reachable_cells(_STARTS["cyan"], walls)
        choices = sorted(shared - starts)
        if len(choices) < _CHECKPOINTS_TOTAL + 10:
            continue
        rng.shuffle(choices)
        checkpoints = choices[:_CHECKPOINTS_TOTAL]
        return {"walls": walls, "checkpoints": checkpoints}
    walls = set(_BORDER_WALLS)
    choices = sorted((_reachable_cells(_STARTS["orange"], walls) & _reachable_cells(_STARTS["cyan"], walls)) - starts)
    rng.shuffle(choices)
    return {"walls": walls, "checkpoints": choices[:_CHECKPOINTS_TOTAL]}
...
def _safe_call(fn: Callable[..., object], x: int, y: int, board: list[list[int]], checkpoint_index: int) -> object:
    try:
        return fn(x, y, board, checkpoint_index)
    except TypeError:
        try:
            return fn(x, y, board)
        except Exception:
...
def _fallback_move(_x: int, _y: int, _board_value: list[list[int]], _checkpoint_index: int = 0) -> str:
    return "stay"
...
def _board(
    ...
    opponent = "cyan" if viewer == "orange" else "orange"
    ox, oy = positions[opponent]
    board[ox][oy] = -2
    return board


def _target_for(slot: str, checkpoints: list[tuple[int, int]], progress: int) -> tuple[int, int]:
    index = min(progress, _CHECKPOINTS_TOTAL - 1)
    if slot == "cyan":
        index = _CHECKPOINTS_TOTAL - 1 - index
...
def _team_ids(context: dict[str, Any]) -> dict[str, str]:
    primary = context.get("team_id")
    return {
        "orange": str(primary) if primary else "team-orange",
        "cyan": "team-cyan",
    }
...
def _frame(
    ...
    targets = {slot: _target_for(slot, checkpoints, progress[slot]) for slot in _SLOTS}
    frame: dict[str, object] = {
        "board": _board(walls, positions, "orange", targets["orange"]),
        "boards": {slot: _board(walls, positions, slot, targets[slot]) for slot in _SLOTS},
        "width": _WIDTH,
        "height": _HEIGHT,
        "positions": {slot: {"x": pos[0], "y": pos[1]} for slot, pos in positions.items()},
        "targets": {slot: {"x": target[0], "y": target[1]} for slot, target in targets.items()},
        "progress": progress,
        "checkpoints_total": _CHECKPOINTS_TOTAL,
        "invalid_moves": invalid,
    }
