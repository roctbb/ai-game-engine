...
def run(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    turns = 0
    frames = [_frame(0, "running", positions, walls, stars, collected, invalid, labels=role_name)]
    ...
    for turn in range(_MAX_TURNS):
        ...
        turns = turn + 1
        frames.append(_frame(turns, "running", positions, walls, stars, collected, invalid, labels=role_name))
    ...
    frames.append(_frame(len(frames), "finished", positions, walls, stars, collected, invalid, slot_scores, labels=role_name))
