...
def run(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    hills = game_map["hills"]
    assert isinstance(walls, set) and isinstance(hills, set)

    positions = dict(_STARTS)
    control = {slot: 0 for slot in _SLOTS}
    invalid = {slot: 0 for slot in _SLOTS}
    turns = 0
    frames = [_frame(0, "running", positions, walls, hills, control, invalid)]
    ...
    for turn in range(_MAX_TURNS):
        ...
        for slot in _SLOTS:
            ...
            x, y = positions[slot]
            action = _safe_call(fn, x, y, _board(walls, hills, positions, slot), slot)
        ...
        if intents["lime"] == intents["navy"]:
            intents = {slot: positions[slot] for slot in _SLOTS}
            events.append({"type": "collision_bounce", "tick": turn + 1})
        elif intents["lime"] == positions["navy"] and intents["navy"] == positions["lime"]:
            intents = {slot: positions[slot] for slot in _SLOTS}
            events.append({"type": "swap_blocked", "tick": turn + 1})

        positions = intents
        for slot in _SLOTS:
            if positions[slot] in hills:
                control[slot] += 1
                events.append({"type": "control", "tick": turn + 1, "slot": slot})
        turns = turn + 1
        frames.append(_frame(turns, "running", positions, walls, hills, control, invalid))

    compile_errors = {slot: err for slot, (_fn, err) in bots.items() if err}
    slot_scores = {slot: control[slot] * 20 - invalid[slot] * 10 for slot in _SLOTS}
    ...
    winner_slot = max(_SLOTS, key=lambda slot: (slot_scores[slot], control[slot]))
    ...
    frames.append(_frame(len(frames), "finished", positions, walls, hills, control, invalid, slot_scores))
...
def _build_map(context: dict[str, Any]) -> dict[str, object]:
    ...
    for _attempt in range(500):
        ...
        shared = sorted((_reachable_cells(_STARTS["lime"], walls) & _reachable_cells(_STARTS["navy"], walls)) - starts)
        if len(shared) < _HILLS_TOTAL + 10:
            continue
        rng.shuffle(shared)
        return {"walls": walls, "hills": set(shared[:_HILLS_TOTAL])}
...
def _board(
    ...
    for x, y in hills:
        board[x][y] = 1
    opponent = "navy" if viewer == "lime" else "lime"
...
def _team_ids(ctx: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        "lime": str(team_id) if isinstance(team_id, str) and team_id else "team-lime",
        "navy": "team-navy",
    }
...
def _frame(
    ...
    frame: dict[str, object] = {
        "board": _board(walls, hills, positions, "lime"),
        "boards": {slot: _board(walls, hills, positions, slot) for slot in _SLOTS},
        "width": _WIDTH,
        "height": _HEIGHT,
