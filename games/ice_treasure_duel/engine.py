...
def run(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    turns = 0
    frames = [_frame(0, "running", positions, walls, crystals, collected, invalid)]
    ...
    for turn in range(_MAX_TURNS):
        ...
        turns = turn + 1
        frames.append(_frame(turns, "running", positions, walls, crystals, collected, invalid))
    ...
    frames.append(_frame(len(frames), "finished", positions, walls, crystals, collected, invalid, slot_scores))
...
def _build_map(context: dict[str, Any]) -> dict[str, object]:
    ...
    if not isinstance(seed, str) or not seed:
        seed = "ice_treasure_duel_offline"
...
def _team_ids(ctx: dict[str, Any]) -> dict[str, str]:
    ...
    return {"ruby": str(team_id) if isinstance(team_id, str) and team_id else "team-ruby", "sapphire": "team-sapphire"}
