...
def run(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    crops = game_map["crops"]
    assert isinstance(walls, set) and isinstance(crops, set)
    ...
    turns = 0
    completed = False
    frames = [_frame(0, "running", position, walls, crops, carrying, delivered, completed, invalid_moves)]

    for turn in range(_MAX_TURNS):
        if completed:
            break
        print_context["tick"] = turn
        action = _safe_call(move_fn, position[0], position[1], _board(walls, crops), carrying)
        ...
        position = target
        turns = turn + 1
        if position in crops and carrying < _CAPACITY:
            crops.remove(position)
            carrying += 1
            events.append({"type": "crop", "tick": turns, "carrying": carrying})
        if position == _BARN and carrying:
            delivered += carrying
            carrying = 0
            events.append({"type": "delivered", "tick": turns, "total": delivered})
        if not crops and carrying == 0:
            completed = True
            events.append({"type": "completed", "tick": turns})
        frames.append(_frame(turns, "running", position, walls, crops, carrying, delivered, completed, invalid_moves))

    score = max(0, delivered * 80 + (300 if completed else 0) - turns * 2 - invalid_moves * 10)
    ...
    frames.append(_frame(len(frames), "finished", position, walls, crops, carrying, delivered, completed, invalid_moves, score))
...
def _build_map(context: dict[str, Any]) -> dict[str, object]:
    ...
    if not isinstance(seed, str) or not seed:
        seed = "garden_harvest_offline"
    rng = random.Random(seed)
    candidates = [(x, y) for y in range(1, _HEIGHT - 1) for x in range(1, _WIDTH - 1) if (x, y) != _BARN]
    for _attempt in range(500):
        rng.shuffle(candidates)
        walls = set(_BORDER_WALLS) | set(candidates[:_RANDOM_WALLS])
        reachable = sorted(_reachable_cells(_BARN, walls) - {_BARN})
        if len(reachable) < _CROPS_TOTAL + 8:
            continue
        rng.shuffle(reachable)
        return {"walls": walls, "crops": set(reachable[:_CROPS_TOTAL])}
...
def _builtins(events: list[dict[str, object]], print_context: dict[str, int]) -> dict[str, object]:
    ...
    return {
        "__import__": safe_import, "abs": abs, "all": all, "any": any, "bool": bool, "dict": dict, "enumerate": enumerate, "float": float, "int": int, "len": len, "list": list, "max": max, "min": min, "print": bot_print, "range": range, "set": set, "str": str, "sum": sum, "tuple": tuple, "zip": zip}


def _safe_call(fn: Callable[..., object], x: int, y: int, board: list[list[int]], carrying: int) -> object:
    try:
...
def _fallback_move(_x: int, _y: int, _board_value: list[list[int]], _carrying: int = 0) -> str:
    return "right"


def _board(walls: set[tuple[int, int]], crops: set[tuple[int, int]]) -> list[list[int]]:
    board = [[0 for _ in range(_HEIGHT)] for _ in range(_WIDTH)]
    for x, y in walls:
        board[x][y] = -1
    for x, y in crops:
        board[x][y] = 1
    board[_BARN[0]][_BARN[1]] = 2
    return board
...
def _frame(tick: int, phase: str, position: tuple[int, int], walls: set[tuple[int, int]], crops: set[tuple[int, int]], carrying: int, delivered: int, completed: bool, invalid_moves: int, score: int | None = None) -> dict[str, object]:
    frame: dict[str, object] = {
        "board": _board(walls, crops),
        "width": _WIDTH,
        "height": _HEIGHT,
        "position": {"x": position[0], "y": position[1]},
        "barn": {"x": _BARN[0], "y": _BARN[1]},
        "carrying": carrying,
        "delivered": delivered,
        "completed": completed,
        "invalid_moves": invalid_moves,
