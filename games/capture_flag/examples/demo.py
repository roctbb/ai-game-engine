def make_move(x, y, board, has_flag):
    target_value = 2 if has_flag else 1
    return step_to(x, y, board, target_value, [("right", 1, 0), ("down", 0, 1), ("left", -1, 0), ("up", 0, -1)])


def step_to(x, y, board, target_value, directions):
    start = (x, y)
    queue = [start]
    ...
    while head < len(queue):
        ...
        head += 1
        cx, cy = current
        if board[cx][cy] == target_value and current != start:
            found = current
            break
