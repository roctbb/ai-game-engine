...
def step_to_value(x, y, board, target_value):
    return bfs_step(x, y, board, find_cells(board, target_value))


def step_next_to_carrier(x, y, board):
    targets = []
    for cx, cy in find_cells(board, -4):
...
def is_open(x, y, board):
    return 0 <= x < len(board) and 0 <= y < len(board[x]) and board[x][y] not in (-4, -3, -2, -1)
