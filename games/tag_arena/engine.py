...
_HEIGHT = 12
_MAX_TURNS = 100
_SLOTS = ("runner", "hunter")
_STARTS = {"runner": (1, 1), "hunter": (10, 10)}
_RANDOM_WALLS = 16
_STARS_TOTAL = 8
_DELTAS = {
    "up": (0, -1),
...
def run(context: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    caught = False
    turns = 0
    frames = [_frame(0, "running", positions, walls, stars, collected, caught, invalid)]

    for turn in range(_MAX_TURNS):
        if caught or not stars:
            break
        ...
        for slot in _SLOTS:
            ...
            x, y = positions[slot]
            action = _safe_call(fn, x, y, _board(walls, stars, positions, slot), slot)
        ...
        if intents["runner"] == intents["hunter"] or (
            intents["runner"] == positions["hunter"] and intents["hunter"] == positions["runner"]
        ):
            caught = True
            positions = intents
            ...
            positions = intents
            if positions["runner"] in stars:
                stars.remove(positions["runner"])
                collected += 1
                events.append({"type": "star", "tick": turn + 1, "count": collected})

        turns = turn + 1
        frames.append(_frame(turns, "running", positions, walls, stars, collected, caught, invalid))

    compile_errors = {slot: err for slot, (_fn, err) in bots.items() if err}
    runner_score = collected * 120 + (200 if not caught else 0) - invalid["runner"] * 10
    hunter_score = (500 if caught else 0) + (_MAX_TURNS - turns if caught else 0) - collected * 25 - invalid["hunter"] * 10
    slot_scores = {"runner": runner_score, "hunter": hunter_score}
    ...
    winner_slot = max(_SLOTS, key=lambda slot: slot_scores[slot])
    ...
    frames.append(_frame(len(frames), "finished", positions, walls, stars, collected, caught, invalid, slot_scores))
...
def _build_map(context: dict[str, Any]) -> dict[str, object]:
    ...
    for _attempt in range(500):
        ...
        shared = sorted((_reachable_cells(_STARTS["runner"], walls) & _reachable_cells(_STARTS["hunter"], walls)) - starts)
...
def _board(
    ...
    for x, y in stars:
        board[x][y] = 1
    opponent = "hunter" if viewer == "runner" else "runner"
...
def _team_ids(ctx: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        "runner": str(team_id) if isinstance(team_id, str) and team_id else "team-runner",
        "hunter": "team-hunter",
    }
...
def _frame(
    ...
    frame: dict[str, object] = {
        "board": _board(walls, stars, positions, "runner"),
